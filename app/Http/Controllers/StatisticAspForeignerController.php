...
//            arsort($files_xls);
        ...
        $studyFormsForInputs = DB::table('study_forms')->join('statistic_asp_foreigners', 'study_forms.id', '=', 'statistic_asp_foreigners.id_studyForm')
...
//        return $studyForms;
        if (isset($studyForms)) {
            ...
            if ($studyForms->count() != 0) {
                return view('pages.stataspforeigner', ['studyForms' => $studyForms, 'faculties' => $faculties,
            ...
            if (isset($faculties) && isset($studyFormsForInputs)) {
                if (($faculties->count() != 0) && ($studyFormsForInputs->count() != 0)) {
                    if (isset($notification)) {
                        ...
                        return view('pages.stataspforeigner', compact('faculties'), compact('studyFormsForInputs'));
                    }
                } else {
                    $faculties = collect(new Faculty);
                    $studyFormsForInputs = collect(new StudyForm);
...
// ----------------поиск по имени-------------------
        if (isset($search_fio)) {
            ...
            $id_students = StudentAspForeigner::where('fio', 'LIKE', '%' . $search_fio . '%')
                ->select('id', 'fio')
                ->get();
            if ($id_students->count() == 0) {
                $id_students = StudentAspForeigner::where('fio_en', 'LIKE', '%' . $search_fio . '%')
                    ->select('id', 'fio')
                    ->get();
            }
            ...
            $statistic_for_people = StatisticAspForeigner::whereIn('id_student', $id_stud_arr)->get();
