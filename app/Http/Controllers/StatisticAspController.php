...
//----------------поиск по категориям-------------------
        ...
        if (!empty($search_faculties)) {
            ...
            $sdf = StatisticAsp::whereIn('id_competition', $id_comp_arr)
...
//                                        return $specializations;
                                ...
                                foreach ($specializations as $kend => $specialization) {
                                    $competitions = CompetitionAsp::whereIn('id', $id_comp_arr)->get();
//
...
//                                            $plan_comp = PlanCompetitionAsp::where('id_competition', '=', $competition->id)->first();
                                        $id_plan_c = PlanCompetitionAsp::where('id_competition', '=', $competition->id)->first();
//                                            $idPlan = $plan_comp->plan;
...
//                                        $freeSeatsNumber = PlanCompetition::where('id_plan', '=', intval($idPlan->id))->first();
                                            $id_plan_comps = PlanCompetitionAsp::where('id_competition', '=', intval($competition->id))->first();
...
//                                        ->first();
        ...
        //Выборка для инпутов
        $studyFormsForInputs = DB::table('study_forms')->join('statistic_asps', 'study_forms.id', '=', 'statistic_asps.id_studyForm')
