...
class StatisticForeignerController extends Controller
{
    use XlsMakerTrait;
...
//            arsort($files_xls);
        ...
        $studyFormsForInputs = DB::table('study_forms')->join('statistic_foreigners', 'study_forms.id', '=', 'statistic_foreigners.id_studyForm')
...
//        return $studyForms;
        if (isset($studyForms)) {
            ...
            if ($studyForms->count() != 0) {
                return view('pages.statforeigner', ['studyForms' => $studyForms, 'faculties' => $faculties,
            ...
            if (isset($faculties) && isset($studyFormsForInputs)) {
                if (($faculties->count() != 0) && ($studyFormsForInputs->count() != 0)) {
                    if (isset($notification)) {
                        ...
                        return view('pages.statforeigner', compact('faculties'), compact('studyFormsForInputs'));
                    }
                } else {
                    $faculties = collect(new Faculty);
                    $studyFormsForInputs = collect(new StudyForm);
...
//----------------поиск по категориям-------------------
        ...
        if (!empty($search_faculties)) {
            $info_faculties = StatisticForeigner::whereIn('id_faculty', $search_faculties)
            ...
            $id_spec_arr = array_unique($id_spec_arr, SORT_REGULAR);
            //var_dump($id_spec_arr);
            $sdf = StatisticForeigner::whereIn('id_competition', $id_comp_arr)
...
//                                        return $specializations;
                                ...
                                foreach ($specializations as $kend => $specialization) {
                                    $competitions = CompetitionForeigner::whereIn('id', $id_comp_arr)->get();
//
                                    foreach ($competitions as $k6 => $competition) {
                                        ...
                                        foreach ($admissionBases as $k3 => $admissionBasis) {
                                            ...
                                            $temp = StatisticForeigner::where('id_studyForm', '=', $studyForm->id)
                                                ...
                                                ->get();
                                            $id_plan_c = PlanCompetitionForeigner::where('id_competition', '=', $competition->id)->first();
...
//                                        $freeSeatsNumber = PlanCompetition::where('id_plan', '=', intval($idPlan->id))->first();
                                                $id_plan_comps = PlanCompetitionForeigner::where('id_competition', '=', intval($competition->id))->first();
...
// ----------------поиск по имени-------------------
        if (isset($search_fio)) {
            ...
            $statistic_for_people = StatisticForeigner::whereIn('id_student', $id_stud_arr)->get();
