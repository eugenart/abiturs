...
//----------------поиск по категориям-------------------
        ...
        if (!empty($search_faculties)) {
            ...
            $sdf = StatisticMasterForeigner::whereIn('id_competition', $id_comp_arr)
...
//                                        return $specializations;
                                ...
                                foreach ($specializations as $kend => $specialization) {
                                    $competitions = CompetitionMasterForeigner::whereIn('id', $id_comp_arr)->get();
//
                                    foreach ($competitions as $k6 => $competition) {
                                        ...
                                        foreach ($admissionBases as $k3 => $admissionBasis) {
                                            ...
                                            $temp = StatisticMasterForeigner::where('id_studyForm', '=', $studyForm->id)
                                                ...
                                                ->get();
                                            $id_plan_c = PlanCompetitionMasterForeigner::where('id_competition', '=', $competition->id)->first();
...
//                                        $freeSeatsNumber = PlanCompetition::where('id_plan', '=', intval($idPlan->id))->first();
                                                $id_plan_comps = PlanCompetitionMasterForeigner::where('id_competition', '=', intval($competition->id))->first();
